...
#------------------------------------------------------------------------------------------------------
# Exemplos de chamada:
# /usr/local/bin/SHOWCast_v_2_5_1/Miniconda3/envs/showcast/bin/python /usr/local/bin/SHOWCast_v_2_5_1/Scripts/process_flood_mapping.py "/dados/fazzt/CIMSS/Flood-Joint/RIVER-FLD-joint-ABI_20240502.part041.tif" -52.0 -5.0 -47.0 0.0 0.375 /usr/local/bin/SHOWCast_v_2_5_1/Output/ /usr/local/bin/SHOWCast_v_2_5_1/HTML/Output/

# python3 process_flood_mapping.py dados/RIVER-FLD-joint-ABI_20240509.part041.tif -52.0 -5.0 -47.0 0.0 0.375 output/ output/
#------------------------------------------------------------------------------------------------------
# Por que o script cria 20 arquivos ao processar uma imagem? Existe algum mecanismo de animação?
#------------------------------------------------------------------------------------------------------
# Required modules
#--------------------------------
...
# O html_update é do próprio ShowCast
...
#------------------------------------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------
def run(CONFIG:list[dict], product:dict): 
    ...
    # Diretorio para arquivos temporários. Adicionado por Bruno.
    ancillaryDir = '/tmp/'

    #------------------------------------------------------------------------------------------------------
    # Adicionado por Bruno, para ajudar a debugar o programa
    gdal.UseExceptions()
    #------------------------------------------------------------------------------------------------------
    ...
    if ("joint" in file_name):
        # Aqui será criado um arquivo FLD_JOINVA_SEC_????????????.webp. Mais tarde, os últimos arquivos
        # serão copiados para FLD_JOINVA_SEC_??.webp a fim de fazer uma animação. Caso não haja arquivos
        # suficientes, os últimos serão copiados até produzir a quantidade desejada de imagens.
    ...
    ancillaryVRTfileName = ancillaryDir + 'flood_' + date_file + '_mosaic.vrt'
    ancillaryTIFfileName = ancillaryDir + 'flood_' + date_file + '_mosaic.tif'
    # Cria um arquivo VRT com todos os TIFFs da data
    gdal.BuildVRT(ancillaryVRTfileName, sorted(glob.glob(path_mosaic), key=os.path.getmtime))
    translate_options = gdal.TranslateOptions(projWin = [extent[0], extent[3], extent[2], extent[1]],  width = sizex, height = sizey)
    # print('translate_options:', translate_options) # não escreve objeto desse tipo
    gdal.Translate(ancillaryTIFfileName, ancillaryVRTfileName, options=translate_options)

    #print(ancillaryTIFfileName)
    #------------------------------------------------------------------------------------------------------
    #------------------------------------------------------------------------------------------------------

    # Read the GeoTIFF as array
    file = gdal.Open(ancillaryTIFfileName)
    ...
    my_logo = plt.imread(str(get_asset_path('noaa_logo.png')))
    ...
    #my_logo = plt.imread(str(get_asset_path("gmu_logo.png")))
    ...
    my_legend = plt.imread(str(get_asset_path("FLOOD_legend.png")))
    ...
    nfiles = 20
    # update(satellite, product, nfiles, outdir, vis_dir)
    ...
    file = None
    os.remove(ancillaryVRTfileName)
    os.remove(ancillaryTIFfileName)
